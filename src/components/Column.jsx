...
export const Column = ({ column, handleDeleteColumn }) => {
  return (
    <section
      className={`flex flex-col items-start justify-center gap-4 rounded-md border-1 border-white px-6 py-4 shadow-md shadow-stone-800 ${column.bgColor}`}
    >
      <header className="flex w-full items-center justify-between border-b-1 border-white text-2xl font-bold">
        <div>
          <h3 className="inline-block w-full p-2 text-white">{column.title}</h3>
        </div>
